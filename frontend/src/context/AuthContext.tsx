...
interface AuthContextType {
  ...
  login: (emailOrUserObj: string | Partial<User>, pass?: string) => Promise<void>;
...
const GUEST_USER: User = {
  id: '', email: '', first_name: 'Guest', last_name: '', name: 'Guest',
  role: 'guest', account_type: 'guest', profile_picture: null, isLoggedIn: false
};
...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [isLoading, setIsLoading] = useState(() => !localStorage.getItem('pixs_user') && !!localStorage.getItem('pixs_token'));
  ...
  const handleAuthSuccess = useCallback((data: any) => {
    ...
    if (loggedInUser.account_type === 'customer') {
      ...
      switch (loggedInUser.role) {
        ...
        case 'staff':
        case 'technician':
        case 'welder': navigate('/staff/overview'); break;
        case 'inventory': navigate('/inventory/overview'); break;
        default: navigate('/homepage');
      }
    }
  }, [navigate]);

  const login = async (emailOrObj: string | Partial<User>, password?: string) => {
    if (typeof emailOrObj === 'object' && emailOrObj !== null) {
      setUser({ ...user, ...emailOrObj, isLoggedIn: true, name: emailOrObj.name || 'Mock User' });
      if (emailOrObj.role === 'customer') navigate('/homepage');
      else if (emailOrObj.role === 'admin') navigate('/admin/dashboard');
      else if (['staff', 'technician', 'welder'].includes(emailOrObj.role as string)) navigate('/staff/overview');
  ...
  return (
    <AuthContext.Provider value={{ 
        user, bannedInfo, isLoading, loading: isLoading, 
        error, fieldErrors, login, register, logout, fetchMe 
    }}>
      {children}
