    ...
    >
      ...
      <div className="flex items-start gap-4">
        <img
          ...
          className="CartProductImage h-24 w-24 rounded-xl object-cover"
        />

        <div className="flex-1 space-y-2">
          <div className="flex items-start justify-between gap-2">
            <div>
              <h3 className="CartProductName text-base font-semibold text-slate-900">
                {item.productName}
              </h3>
              <p className="CartProductVariant text-sm text-slate-600">
                Variant: {item.variant.size} ({item.variant.width} x{' '}
                {item.variant.height})
              </p>
            </div>
            <button
          ...
          <PlateInfo
            plate={item.plate}
            setupFeeApplied={totals.setupFeeApplied}
          />
          ...
          <div className="grid grid-cols-2 gap-2 rounded-xl bg-slate-50 p-3 text-xs text-slate-600 md:grid-cols-4">
            ...
            <div>
              <p>Print Price</p>
              <p className="font-semibold text-slate-900">
                PHP {(item.plate?.printPricePerUnit ?? 0).toFixed(2)}
              </p>
            </div>
            <div>
              <p>Setup Fee</p>
              <p className="font-semibold text-slate-900">
                PHP {totals.setupFeeApplied.toFixed(2)}
              </p>
            </div>
            <div>
              <p>Total</p>
