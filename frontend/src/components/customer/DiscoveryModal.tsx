...
import { Search, ArrowLeft, Heart, Plus, Package, X, RotateCcw } from 'lucide-react'
...
const DiscoveryModal: React.FC<{
  ...
  const { categories, isLoading: catLoading, error: catError } = useDiscoveryCategories(isOpen)
  const debouncedQuery = useDebounce(query, 350)
  const { results: filteredResults, isLoading: searchLoading, error: searchError } = useDiscoverySearch({
        ...
        <div className="sticky top-0 z-50 flex items-center gap-4 border-b border-slate-50 bg-white/80 p-6 backdrop-blur-md">
          ...
          <div className="group relative flex-1">
            ...
            <input
              ...
              placeholder={selectedCategory ? `Search in ${categories.find(c => c.id === selectedCategory)?.label}...` : "SKU Code / Keyword Search Protocol"}
              ...
              {catLoading && (
                <div className="grid w-full grid-cols-2 gap-4 md:gap-8">
                  {Array.from({ length: 4 }).map((_, i) => (
                    <div
                      key={i}
                      className="animate-pulse aspect-square w-full rounded-[32px] bg-slate-100 md:rounded-[48px] lg:aspect-[21/9] lg:h-64"
                    />
                  ))}
                </div>
              )}
                  ...
                  {categories.map((cat) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        setQuery('') // Clear query to show ALL products in category as requested
                      }}
                    ...
                    >
                      <img
                        ...
                        decoding="async"
                        onError={(e) => {
                          (e.target as HTMLImageElement).src = '/assets/fallback-category.png'
                        }}
              ...
              <div className="flex items-end justify-between border-b border-slate-50 pb-8">
                <div className="space-y-2">
                  <p className="text-[10px] font-black tracking-widest text-slate-400 uppercase">
                    Projection Active {selectedCategory && `· ${categories.find(c => c.id === selectedCategory)?.label}`}
                  </p>
                ...
                {searchError && (
                  <div className="flex items-center gap-4">
                    <p className="text-[10px] font-bold text-red-400 uppercase tracking-widest">{searchError}</p>
                    <button 
                      onClick={() => window.location.reload()}
              ...
              {searchLoading && (
                <div className="grid grid-cols-2 gap-4 md:grid-cols-3 md:gap-10 lg:grid-cols-4">
                  {Array.from({ length: 8 }).map((_, i) => (
                    <div key={i} className="animate-pulse rounded-[32px] bg-slate-100 md:rounded-[44px]" style={{ aspectRatio: '1 / 1.4' }} />
                  ))}
                </div>
              )}
                  ...
                  {filteredResults.map((product) => (
                    ...
                    >
                      ...
                      <div className="flex flex-1 flex-col px-4 pb-4">
                        ...
                        <Link to={`/product/${encodeURIComponent(product.id)}`} onClick={onClose}>
