...
export const passwordSchema = z
  ...
  .regex(
    /[^A-Za-z0-9]/,
    'Constraint Violation: Missing Special Character Node.',
  )
  .refine(
    (val) => !val.includes(' '),
    'Constraint Violation: Whitespace logic detected.',
  )
...
export const getStrengthLabel = (strength: number) => {
  ...
  if (strength < 80)
    return { label: 'VULNERABLE', color: 'text-amber-500', bar: 'bg-amber-500' }
  return {
    label: 'REINFORCED',
