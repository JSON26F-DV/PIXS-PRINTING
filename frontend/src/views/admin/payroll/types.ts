...
export const calculateDailySalary = (day: Partial<AttendanceDay>, dailyRate: number, otRate: number): number => {
  ...
  return Math.max(0, (percentage * appliedDailyRate) + (overtime * otRate) - lateDeduction);
};

export const generateEmptyWeek = (startDate: Date, baseRate: number = 0): AttendanceDay[] => {
