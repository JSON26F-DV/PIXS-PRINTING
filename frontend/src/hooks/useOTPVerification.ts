...
export const useOTPVerification = () => {
  ...
  const sendCode = useCallback(
    (targetMethod: 'email' | 'sms') => {
      ...
      console.log(
        `[SECURITY PROTOCOL] OTP generated for ${targetMethod}: ${code}`,
      )
    ...
    (inputOTP: string) => {
      ...
      if (inputOTP === generatedOTP) {
        ...
        if (newAttempts >= MAX_ATTEMPTS) {
          setIsLocked(true)
          toast.error(
            'CRITICAL: Terminal locked due to multiple invalid nodes.',
          )
          ...
          toast.error(
            `Invalid node sequence. ${MAX_ATTEMPTS - newAttempts} attempts remaining.`,
          )
        }
        return false
      }
